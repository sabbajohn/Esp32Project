...
static int32_t bin_raw_round_nearest(float value) {
...
static bool
    ...
    if(bin_raw_type == BinRAWTypeGap) {
        ...
        do {
            gap_ind--;
            data_temp = bin_raw_round_nearest((float)(instance->data_raw[gap_ind]) / instance->te);
        ...
        for(size_t i = 0; i < instance->data_raw_ind; i++) {
            int data_temp = (int)bin_raw_round_nearest((float)(instance->data_raw[i]) / instance->te);
